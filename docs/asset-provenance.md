...
| `docs/patchoath-dashboard.webp` | Captured by the project's own Chromium E2E from the tested PatchOath Dashboard on `main` (workflow run `34039893849`, browser-evidence artifact) | README-optimized derivative of the real 1600×2596 E2E PNG. Processing was limited to resizing and WebP re-encoding; product content, labels, layout, and evidence shown were not manually composited or generatively altered. |
...
The retired pre-v0.3 vector mark and the earlier `docs/vibetrace-dashboard.png` interface capture have been removed from the current tree rather than retained as reusable assets. Their history remains available through Git if provenance review is ever needed.
...
New launch material should use `docs/patchoath-mark.svg` and the tested `docs/patchoath-dashboard.webp` capture. Retired-brand assets may remain in Git history or explicit migration documentation, but they should not be reintroduced into new marketing surfaces.

The current README hero is deliberately derived from a successful automated browser run rather than a manually composed product mockup. If the Dashboard changes materially, regenerate the hero from a successful current E2E run so the public image continues to reflect tested product behavior.
